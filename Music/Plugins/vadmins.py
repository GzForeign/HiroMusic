...
async def cbmenu(_, query: CallbackQuery):
    if query.message.sender_chat:
        return await query.answer(
            "Lu **Admin Anonim** !\n\n» balik ke akun pengguna yang lu admin."
        )
    a = await _.get_chat_member(query.message.chat.id, query.from_user.id)
    if not a.can_manage_voice_chats:
        return await query.answer(
            "🦍 Cuma admin dengan izin mengelola obrolan suara yang bisa mencet tombol ini !",
            show_alert=True,
        )
    await query.edit_message_text(
        f"⚙️ **Pengaturan dari** {query.message.chat.title}\n\nII : Jeda Streaming\n▷ : Lanjutin Streaming\n🔇 : Bisukan Asisten\n🔊 : Bunyikan Asisten\n▢ : Berhentiin Streaming",
...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            await m.reply("❌ Gada yang lagi diputar")
        elif op == 1:
            await m.reply(
...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply("✅ **Streaming dihentikan.**")
        except Exception as e:
            await m.reply(f"**Error:**\n\n`{e}`")
    else:
        await m.reply("❌ **Gada streaming**")
...
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "II **Video dijeda.**\n\n• **Untuk melanjutkan video, gunain Perintah** » /vresume"
            )
        except Exception as e:
            await m.reply(f"**Error:**\n\n`{e}`")
    else:
...
async def resume(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "▷ **Video dilanjutin.**\n\n• **Untuk menjeda video, gunain Perintah** » /vpause"
            )
        except Exception as e:
            await m.reply(f"**Error:**\n\n`{e}`")
    else:
...
async def mute(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "🔇 **Asisten dibisuin.**\n\n• **Untuk mengaktifkan suara Assistant, gunain Perintah**\n» /vunmute"
            )
        except Exception as e:
            await m.reply(f"**Error:**\n\n`{e}`")
    else:
...
async def unmute(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "🔊 **Asisten dibunyikan.**\n\n• **Untuk menonaktifkan bot pengguna, gunain Perintah**\n» /vmute"
            )
        except Exception as e:
            await m.reply(f"**Error:**\n\n`{e}`")
    else:
...
async def cbpause(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.pause_stream(chat_id)
            await query.edit_message_text("II Streaming dijeda", reply_markup=bttn)
        except Exception as e:
            await query.edit_message_text(f"**Error:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ Gada yang lagi streaming", show_alert=True)
...
async def cbresume(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.resume_stream(chat_id)
            await query.edit_message_text(
                "▷ Streaming dilanjutin", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"**Error:**\n\n`{e}`", reply_markup=bcl)
    else:
...
async def cbstop(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await query.edit_message_text(
                "✅ **Streaming dah selesai**", reply_markup=bcl
            )
        except Exception as e:
            await query.edit_message_text(f"**Error:**\n\n`{e}`", reply_markup=bcl)
    else:
...
async def cbmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.mute_stream(chat_id)
            await query.edit_message_text(
                "🔇 Assistant berhasil dibisuin", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"***Error:**\n\n`{e}`", reply_markup=bcl)
    else:
