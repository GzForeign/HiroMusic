...
async def pausevc(_,CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            await CallbackQuery.message.reply(f"🎧 Voicechat Paused by {rpk}!")
            await CallbackQuery.message.delete()
        else:
...
async def resumevc(_,CallbackQuery):  
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            user_name = CallbackQuery.from_user.first_name
            rpk = "["+user_name+"](tg://user?id="+str(user_id)+")"
            await CallbackQuery.message.reply(f"🎧 Voicechat Resumed by {rpk}!")
            await CallbackQuery.message.delete()
    else:
        await CallbackQuery.answer(f"❌ Tidak ada yang diputar di Musik", show_alert=True)
...
async def skipvc(_,CallbackQuery): 
    ...
    if await is_active_chat(chat_id):
        ...
        if is_empty(chat_id):
            ...
            finxx = (f"{f1}{f2}{f3}")
            if str(finxx) != "raw":   
                mystic = await CallbackQuery.message.reply("Musik sedang diputar Daftar Playlist...\n\nDownloading Musik Berikutnya Dari Daftar Playlist....")
                ...
                caption=(f"<b>Skipped Voice Chat By {rpk}</b>\n\n🎥 <b>Started Playing : </b>[{title[:25]}]({url}) \n⏳ <b>Duration :</b> `{duration}` `Menit`\n👤**Requested by :** {semx.mention}")
            )   
                os.remove(thumb)
            else:      
                await music.pytgcalls.change_stream(
...
async def play_playlist(_,CallbackQuery):
    ...
    if str(smex) == "personal":
        if CallbackQuery.from_user.id != int(user_id):
            return await CallbackQuery.answer("❌ Ini bukan untukmu! Mainkan daftar putar Anda sendiri", show_alert=True)
...
Personal Playlist Playing."""
            ...
            for note in _playlist:
                ...
                if await is_active_chat(chat_id):
                    ...
                    await CallbackQuery.message.delete()
        await mystic.delete()
        m = await CallbackQuery.message.reply_text("Menempel Daftar Putar Antrian ke Bin")
    ...
    if str(smex) == "group":
        ...
        if not _playlist:
            return await CallbackQuery.answer(f"❌ Grup Anda tidak memiliki daftar putar di server, Coba tambahkan musik di daftar playlist.", show_alert=True)
        else:
            await CallbackQuery.message.delete()
            logger_text=f"""Starting Playlist
...
Group Playlist Playing."""
        ...
        await mystic.delete()
        m = await CallbackQuery.message.reply_text("Menempel Daftar Playlist Antrian ke Bin")
        link = await paste(msg)
        preview = link + "/preview.png"
        urlxp = link + "/index.txt"
        a1 = InlineKeyboardButton(text=f"Checkout Queued Playlist", url=urlxp)
...
async def group_playlist(_,CallbackQuery):
    ...
    count = 0
    if not _count:
        sex = await CallbackQuery.message.reply_text("Selamat Datang Di Fitur Daftar Playlist Musik.\n\Membuat Daftar Playlist Grup Anda di Basis Data... Harap tunggu.")
    ...
    count = int(count)
    if count == 30:
        return await CallbackQuery.message.reply_text("❌ Maaf Anda hanya dapat memiliki 30 musik di daftar playlist grup.")
    try:
    ...
    title = title[:50]
    if _check:
         return await CallbackQuery.message.reply_text(f"{Name}, Sudah ada di Daftar Playlist")   
    assis = {
        "videoid": videoid,
        "title": title,
        "duration": duration,
    ...
    Name = CallbackQuery.from_user.first_name
    return await CallbackQuery.message.reply_text(f"✅ Ditambahkan ke Daftar Playlist Grup oleh {Name}")
...
async def pla_playylistt(_,CallbackQuery):
    ...
    count = 0
    if not _count:
        sex = await CallbackQuery.message.reply_text("Selamat Datang di Fitur Daftar Playlist Musik.\n\nMembuat Daftar Playlist Anda Di Basis Data... Harap tunggu.")
        await asyncio.sleep(2)
        await sex.delete()
    else:
        for smex in _count:
    ...
    if count == 30:
        if userid in SUDOERS:
            ...
            return await CallbackQuery.message.reply_text("❌ Maaf Anda hanya dapat memiliki 30 musik di daftar playlist Anda.")
    try:
        url = (f"https://www.youtube.com/watch?v={url}")
        results = VideosSearch(url, limit=1)
        for result in results.result()["result"]:
...
async def P_list(_,CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(f"❌ Anda tidak memiliki Daftar Playlist Pribadi di server, Coba tambahkan musik di daftar playlist.", show_alert=True)
    else:
        j = 0
        await CallbackQuery.answer()
...
async def G_list(_,CallbackQuery):
    ...
    _playlist = await get_note_names(CallbackQuery.message.chat.id)
    if not _playlist:
        return await CallbackQuery.answer(f"❌ Anda tidak memiliki Daftar playlist Grup di server, Coba tambahkan musik di daftar playlist", show_alert=True)
    else:
        await CallbackQuery.answer()
        j = 0
